...
impl SteensgaardState {
    ...
    /// Union-by-rank merge.  Merging into `Unknown` redirects the other
    /// node's parent; if both `Id` roots have a `points_to`, join those too.
...
impl AliasResult {
    /// The simplest possible aliasing result: each value aliases only itself and other values in the same space.
    pub fn from_space_ids(ctx: &Context) -> Self {
        let mut value_to_root: HashMap<_, _> = ctx
            .values
            .varnodes
            .iter()
            .map(|varnode| {
                (
                    varnode.id.into(),
                    NodeId::Id(usize::from(Varnode::from_id(ctx, varnode.id).space().id)),
                )
            })
            .collect();

        // Handle the register space more precisely
        // TODO: some registers may collide !
        // TODO: Copy is not handled well
        // TODO: else does not add to block!
        for &reg in ctx.registers.values() {
            value_to_root.insert(reg.into(), NodeId::Id(ctx.spaces.len() + usize::from(reg)));
        }
...
mod tests {
    ...
    // 1. zext propagates alias: r = zext(ptr) → may_alias(r, ptr)
    #[test]
    ...
    // 2. Load-store round-trip: store v → *ptr; r = load(*ptr) → may_alias(r, v)
    #[test]
    ...
    // 3. Distinct unrelated values do not alias
    #[test]
    ...
    // 4. Binop propagates alias: r = ptr + offset → may_alias(r, ptr)
    #[test]
    ...
    // 5. Comparison does NOT propagate: cond = (a == b) → !may_alias(cond, a)
    #[test]
    ...
    // 6. Transitive join: store v → *p; p_val = load(*p); store p_val → *q; final = load(*q)
    //    → may_alias(final, v)
    #[test]
    ...
    // 8. caller_saved register is joined to Unknown after Call with signature
    #[test]
    ...
    // 9. outputs register is joined to Unknown after Call with signature
    #[test]
    ...
    // 10. No signature → no alias effect on registers
    #[test]
    ...
    // 11. CallInd → no alias effect regardless of register
    #[test]
    ...
    // 7. Two loads through the same pointer value alias each other
    #[test]
