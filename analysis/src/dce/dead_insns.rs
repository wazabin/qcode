...
use qcode::value::{BlockId, FunctionId, InstructionId, QCodeView, ValueId, insn::Mnemonic};

use qcode_passes::{
    dce::{has_users, host_users, remove_dead_insns_body},
    replace_terminator_with_branch,
};

// The block-local core — `dead_insns`, `remove_dead_insns` and the no-predecessor
// param sweep — lives in `qcode_passes` so the VM's lifter can run it without
// depending on the analysis pipeline. What stays here is the machinery that
// needs the rest of analysis: dead pure calls and dead counted loops.
pub use qcode_passes::dce::{dead_insns, remove_dead_insns};
...
pub fn remove_dead_pure_call(ctx: &mut Context, block_id: BlockId) -> bool {
    crate::with_body_mut(ctx, block_id.func, |body, cx| {
        remove_dead_pure_call_body(body, cx, block_id)
    })
}

/// Body-local core of [`remove_dead_pure_call`].
fn remove_dead_pure_call_body<'a, 'str>(
    ...
    replace_terminator_with_branch(body, cx.pass(), block_id, *fallthrough, vec![]);
    true
}
...
fn remove_dead_counted_loop(ctx: &mut Context, fun_id: FunctionId) -> bool {
    crate::with_body_mut(ctx, fun_id, |body, cx| {
        remove_dead_counted_loop_body(body, cx, fun_id)
    })
}

/// Body-local core of [`remove_dead_counted_loop`].
fn remove_dead_counted_loop_body<'a, 'str>(
    body: &'a mut FunctionBody<'str>,
    cx: ContextView<'a, 'str>,
    fun_id: FunctionId,
) -> bool {
    ...
    for header in headers {
        if let Some(dl) = match_dead_loop(cx.body_view(body), header) {
            replace_terminator_with_branch(body, cx.pass(), dl.preheader, dl.exit, vec![]);
            return true;
        }
    }
    false
...
/// Body-local core of the [`Dce`] pass: the fixpoint over per-block dead-pure-call
...
fn dce_core<'a, 'str>(
    ...
    loop {
        ...
        for &block_id in &block_ids {
            ...
            round |= remove_dead_insns_body(body, cx.pass(), block_id);
        }
