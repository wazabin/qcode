...
fn apply_unroll_plan<'a, 'str>(
    ...
    for iteration in 0..plan.lp.iterations {
        ...
        for (path_index, &old_block) in plan.path.iter().enumerate() {
            ...
            if let Some(previous) = previous_new_block {
                replace_terminator_with_branch(body, cx.pass(), previous, new_block, Vec::new());
            }
    ...
    if let Some(last_new_block) = previous_new_block {
        ...
        replace_terminator_with_branch(body, cx.pass(), last_new_block, plan.exit, exit_args);
    }
...
pub(crate) use qcode_passes::replace_terminator_with_branch;
