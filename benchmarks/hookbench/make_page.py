...
<li><b>Where the uninstrumented run's time goes.</b> On these short images a third to a half of the QCode JIT's run is Cranelift compiling blocks, and on block-hopping code (nsichneu) the per-block round trip through the executor is twice the time spent in the compiled code itself; the hooks add their operations on top, which is what the instruction ratios in the progress table isolate. The warm-up run is the JIT compiling a block on its second entry rather than its first, so a block still growing as the lifter discovers the code after it is not compiled after each absorbed instruction, and code that runs once is not compiled at all: the base run got 1.27× faster and the compiled hooks' ratios did not move.</li>
