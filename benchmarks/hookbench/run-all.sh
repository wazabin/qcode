...
mkdir -p "$OUT"
uptime > "$OUT/load.txt"
"$BIN" --images "$IMAGES" --engine qcode-jit --repeat "$REPEAT" --json "$OUT/qcode-jit.json" > "$OUT/qcode-jit.log" 2>&1
"$BIN" --images "$IMAGES" --engine unicorn --repeat "$REPEAT" --json "$OUT/unicorn.json" > "$OUT/unicorn.log" 2>&1
"$BIN" --images "$IMAGES" --engine icicle --repeat "$REPEAT" --json "$OUT/icicle.json" > "$OUT/icicle.log" 2>&1
